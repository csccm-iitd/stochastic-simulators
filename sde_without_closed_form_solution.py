# -*- coding: utf-8 -*-
"""SDE_without_closed_form_solution
"""
...
dataset = pd.read_csv('SDE60X50.csv')
X = dataset.iloc[:, :-1].values
y = dataset.iloc[:, -1].values

dataset_val = pd.read_csv('SDE30X10.csv')
...
# Training Data
x1_train = np.array(X[:,0],dtype= np.float32)
x2_train = np.array(X[:,1],dtype= np.float32)
y_train  = np.array(y,dtype= np.float32)

#Validation Data
x1_valid = np.array(X_val[:,0],dtype= np.float32)
x2_valid = np.array(X_val[:,1],dtype= np.float32)
...
val_dataset = tf.data.Dataset.from_tensor_slices((x1_valid,x2_valid, y_valid))
val_dataset = val_dataset.shuffle(buffer_size=600).batch(batch_size)

dataset1 = pd.read_csv('validation_sde_x1_1point2_x2_point3.csv')
yactual1 = dataset1.iloc[:, 0].values

"""Network Definition and Compilation"""

n_v_r = 60 # Number of normal random variables
input_size = n_v_r + 2
...
def kde_gaussian_extra(y_d,y_a,bd):   
  kde1 = kd(bandwidth=bd, kernel='gaussian')
  kde1.fit(y_a)
  logprob1 = kde1.score_samples(y_d[:,None])
  value_a = tf.constant(np.exp(logprob1), dtype=tf.float32)
  return value_a

x1_value = 1.2
x2_value = 0.3
y_pred = testfunction(x1_value, x2_value,2000)
yactual1 = yactual1.reshape([2000,1])
y_d1 = np.linspace(0,2.5,1500)
